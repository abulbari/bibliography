...
if (isset($_SESSION['login_username'])) 
{
    ...
    //fetch all the detail for add or update
    $id=$_POST['id'];
    ...
    //if you selected to add a new entry in the library you want
    if ($_POST['action'] == 'AddNew') 
    {
        $sql = "INSERT INTO ReferenceLibrary (Email,LibName,Title,Author,Year,Abstract)"
                . "VALUES ('$user','$library','$title','$author','$year','$abstract')";
        ...
        {
            echo "<script>alert('New Data record added in the Table');</script>\n";
        }
        $dbh = new PDO("mysql:dbname=test;host=localhost;", "root", "12345");
        $statement = $dbh->prepare("SELECT Libname FROM Library where Libname=?");
        $statement->execute(array($library));
        $result = $statement->fetchAll();
        //if the Libraty exist then Data gets Added in this library;
        if ($result) 
        {
            
        } 
        //create a new library and then insert it
        else 
        {

            $sql1 = "INSERT INTO Library (Email,LibName)"
                    . "VALUES ('$user','$library')";
    ...
    //if user choose to update then he can update with ID
    else
    if ($_POST['action'] == 'Update') 
    {
        if ($library != '') 
        {
            $query1 = "UPDATE ReferenceLibrary SET LibName='$library' WHERE Id='$id'";
            mysqli_query($con, $query1);
        }
        if ($title != '') {
            $query3 = "UPDATE ReferenceLibrary SET Title='$title' WHERE Id='$id'";
            mysqli_query($con, $query3);
        }
        if ($author != '') {
            $query4 = "UPDATE ReferenceLibrary SET Author='$author' WHERE Id='$id'";
            mysqli_query($con, $query4);
        }
        if ($abstract != '') {
            $query5 = "UPDATE ReferenceLibrary SET Abstract='$abstract' WHERE Id='$id'";
            mysqli_query($con, $query5);
        }
        if ($year != '') {
            $query6 = "UPDATE ReferenceLibrary SET Year='$year' WHERE Id='$id'";
            mysqli_query($con, $query6);
        }
        echo "<script>alert('New Data record updated in the Table');</script>\n";
    }
